...
- Handshake: `handshake/hello` performed once per session (idempotent disk cache). ServerHello signature-verified, bound to the ClientHello, and its `on_decision_failure` and `timeout_config` participate in the effective posture/timeout (most-restrictive-wins with `ACS_DEFAULT_DENY`; §6.4:154 negotiated timeout). Its `methods_evaluated` bounds what is sent: a method the Guardian omits resolves as ALLOW-by-default without being sent (handshake.json), recorded as `method_not_evaluated`; permission events and `beforeSubmitPrompt` return their explicit allow shapes, since `failClosed` reads an empty reply as a failed hook. With no negotiated list, every mapped event is sent. Failures negative-cached so a dead Guardian costs one timeout, not one per event.
