...
External verification may include establishing that the bootstrap — Guardian policy, initial Intent, and Approver configuration — was adopted or authorized through an accountable process, and that any claimed human or institutional authority is supported by a documented delegation chain. The ASK intent-extension flow ([§9.1](./instrument/specification.md#91-intent-extension-via-ask-normative)) can conformantly widen `Intent.parsed` in-session; it may exercise or extend authority already recognized by that external process, but protocol operation alone cannot create the source of institutional authority. Where such authorization exists, ACS-Trace and ACS-Audit are well-suited to record it.
